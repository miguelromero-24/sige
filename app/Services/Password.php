...
class Password
{
    ...
    public function validatePassword($password)
    {
        ...
        if (strlen($password) < $this->minLength) {
            $this->errors[] = 'La contraseña debe tener ' . $this->minLength . ' caractéres como mínimo, la ingresada es muy corta con ' . strlen($password) . ' caracteres.';
            return false;
        }
        // Check password maximum length, return at this step.
        if (strlen($password) > $this->maxLength) {
            $this->errors[] = 'La contraseña debe tener ' . $this->maxLength . ' caractéres como máximo, la ingresada es muy larga con ' . strlen($password) . ' caracteres.';
            return false;
        }
        // Check the number of numbers in the password.
        if (strlen(preg_replace('/([^0-9]*)/', '', $password)) < $this->minNumbers) {
            $this->errors[] = 'Cantidad insuficiente de números en la contraseña, se requiere un mínimo de ' . $this->minNumbers . ' números.';
        }
        // Check the number of letters in the password
        if (strlen(preg_replace('/([^a-zA-Z]*)/', '', $password)) < $this->minLetters) {
            $this->errors[] = 'Cantidad insuficiente de letras en la contraseña, se requiere un mínimo de ' . $this->minLetters . ' letras.';
        }
        ...
        if (strlen(preg_replace('/([^a-z]*)/', '', $password)) < $this->minLowerCase && $this->minLowerCase != 0) {
            $this->errors[] = 'Cantidad insuficiente de minúsculas en la contraseña, se requiere un mínimo de ' . $this->minLowerCase . ' minúsculas.';
        }
        ...
        if (strlen(preg_replace('/([^A-Z]*)/', '', $password)) < $this->minUpperCase && $this->minUpperCase != 0) {
            $this->errors[] = 'Cantidad insuficiente de mayúsculas en la contraseña, se requiere un mínimo de ' . $this->minUpperCase . ' mayuscúlas.';
        }
        ...
        if (strlen(preg_replace('/([a-zA-Z0-9]*)/', '', $password)) < $this->minSymbols && $this->maxSymbols != 0) {
            $this->errors[] = 'Cantidad insuficiente de símbolos en la contraseña, se requiere un mínimo de ' . $this->minSymbols . ' símbolos.';
        }
        ...
        if (strlen(preg_replace('/([a-zA-Z0-9]*)/', '', $password)) > $this->maxSymbols) {
            if ($this->maxSymbols == 0) {
                $this->errors[] = 'No se permiten símbolos en la contraseña.';
            } else {
                $this->errors[] = 'El número de símbolos exede la cantidad máxima permitida de ' . $this->maxSymbols . ' símbolos.';
            }
        }
        ...
        if ($this->maxSymbols != 0) {
            ...
            for ($i = 0; $i < strlen($symbols); ++$i) {
                if (!in_array($symbols[$i], $this->allowedSymbols)) {
                    $this->errors[] = 'Símbolo no permitido:  ' . $symbols[$i] . ' , solo se permiten los siguientes: ' . implode('', $this->allowedSymbols) . '.';
                }
            }
        }
